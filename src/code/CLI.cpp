...
static string readFileBinary(const string& path) {
    ...
    if (!file.is_open()) throw runtime_error("Cannot open input file: " + path);
...
static void writeFileBinary(const string& path, const string& data) {
    ...
    if (!file.is_open()) throw runtime_error("Cannot open output file: " + path);
...
static Args parseArgs(int argc, char** argv) {
    if (argc < 2) throw runtime_error("Missing command. Use --help");
    ...
    if (a.inPath.empty()) throw runtime_error("Missing -i <input>.");
...
static void saveDictMeta(const string& metaPath, const DictType& dict) {
    ...
    if (!f.is_open()) throw runtime_error("Cannot write meta file: " + metaPath);
...
static DictType loadDictMeta(const string& metaPath) {
    ...
    if (!f.is_open()) throw runtime_error("Cannot open meta file: " + metaPath);
...
int run_cli(int argc, char** argv) {
    try {
        ...
        if (args.cmd == "verify") {
            ...
            if (args.outPath.empty()) cout << report << "\n";
            else writeFileText(args.outPath, report);
            return 0;
        }
...
if (args.cmd == "compress") {
    ...
    if (!out.is_open()) throw runtime_error("Cannot open output file: " + args.outPath);
...
if (args.cmd == "decompress") {
    ...
    if (!in.is_open()) throw runtime_error("Cannot open input file: " + args.inPath);
